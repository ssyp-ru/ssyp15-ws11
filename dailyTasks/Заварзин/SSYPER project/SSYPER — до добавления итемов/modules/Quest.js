var Quest=function(qnm,qstn,qendtm,qtp,qatp,qm,qfm,qaw,tw)
{
		...
		for (var gai=0;gai<this.Award[gaj].length;gai++)
			switch (typeof (this.Award[gaj][gai]))
			{
				...
				case "string":
				{
					...
					{
						this.Award[gaj][gai]=this.Award[gaj][gai].slice(1);
						character.Items.splice(GetIndexByName(this.Award[gaj][gai],character.Items),1);
					}
					else
	...
	this.StartCheck=function(character)
	{
		scresult=true;
