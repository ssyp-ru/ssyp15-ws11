...
function getBestQuest() {
	...
	for(var c in quests.quests) {
		...
		{
			if(quests.quests[c].priority>maxP & !quests.quests[c].npc)
...
quests.register(new Quest("Иди оденься по погоде",["mainHouse"],3,true,[0,0,0,0],null));
quests.register(new Quest("Иди помойся,маня",["mainHouse"],3,true,[0,0,0,0],null));
