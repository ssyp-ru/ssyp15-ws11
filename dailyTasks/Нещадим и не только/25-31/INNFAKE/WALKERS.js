...
Walker.prototype.addCompQ=function(name) {
  this.compQ.push(name);
};
Walker.prototype.resetCompQ=function(){
    this.compQ=[];
};
Walker.prototype.getCofQ=function(name) {
    var res=0;
    for(var c in this.compQ) {
        if (typeof (this.compQ[c]) === "string")
        {
            if(this.compQ[c]===name)
            res++;
        }
    }
    return res;
...
Walker.prototype.addXP=function(xp) {
    this.xp+=xp;
    while(this.xp>99)
    {
        this.lv++;
...
Walker.prototype.addHp=function(hp) {
    this.hp+=hp;
    if(this.hp>100)
    this.hp=100;
}
...
function saveWalkers( ) {
 ...
 for(var i in walkers) {
    ...
    for( var j in walkers[i].targets)
    {
     ...
     w.compQ=[];
    // console.log(walkers[i].compQ);
     for( var g in walkers[i].compQ) {
         if(typeof (walkers[i].compQ[g])!=="function") {
             w.compQ[g] = walkers[i].compQ[g];
       //     console.log("добавил");
         }
     }
     ...
     for(var c in walkers[i]) {
         if(!(c in w) && typeof (walkers[i][c])!=="function")
         {
             w[c]=walkers[i][c];
         }
     }
   ...
   /// console.log(w);
    ...
    // w.compQ=JSON.stringify(w.compQ);
    // console.log(w.compQ);
     //console.log(JSON.stringify(w.compQ));
     saved[walkers[i].id]= JSON.stringify(w);
    // console.log(saved[walkers[i].id]);
 }
 //console.log(str);
    //console.log(saved);
    var str="{";
